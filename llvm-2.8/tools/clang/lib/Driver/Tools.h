...
namespace visualstudio {
 ...
 /// Rigel toolchain, based on GNU Binutils 2.18
namespace rigel {
  class LLVM_LIBRARY_VISIBILITY Assemble : public Tool  {
  public:
    Assemble(const ToolChain &TC) : Tool("rigel::Assemble", "assembler",
                                         TC) {}
    ...
    const toolchains::RigelToolChain &getRigelToolChain() const {
      return reinterpret_cast<const toolchains::RigelToolChain&>(getToolChain());
    }
  };
  class LLVM_LIBRARY_VISIBILITY Link : public Tool  {
  public:
    Link(const ToolChain &TC) : Tool("rigel::Link", "linker", TC) {}
    ...
    virtual void ConstructJob(Compilation &C, const JobAction &JA,
                              ...
                              const ArgList &TCArgs,
                              const char *LinkingOutput) const;
    //TODO: This code is duplicated in the Assemble and Link classes.
    //Darwin does it by having a single DarwinTool class that the Assemble, Link, ...
    //tools inherit from.  We could do that.
    const toolchains::RigelToolChain &getRigelToolChain() const {
      return reinterpret_cast<const toolchains::RigelToolChain&>(getToolChain());
    }
  };
