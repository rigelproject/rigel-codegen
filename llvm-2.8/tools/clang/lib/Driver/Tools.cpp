...
void rigel::Assemble::ConstructJob(Compilation &C, const JobAction &JA,
                                       const InputInfo &Output,
                                       const InputInfoList &Inputs,
                                       const ArgList &Args,
                                       const char *LinkingOutput) const {
  ...
  CmdArgs.push_back("-EL");
  CmdArgs.push_back("-march=mipsrigel32");
  ...
  const char *Exec =
    Args.MakeArgString(getToolChain().GetProgramPath("rigelas"));
  C.addCommand(new Command(JA, *this, Exec, CmdArgs));
}

void rigel::Link::ConstructJob(Compilation &C, const JobAction &JA,
                                   const InputInfo &Output,
                                   const InputInfoList &Inputs,
                                   const ArgList &Args,
                                   const char *LinkingOutput) const {
  ...
  //TODO No idea if -Bstatic, -Bshareable, and -dynamic-linker work for us.
  /*
  if (Args.hasArg(options::OPT_static)) {
    CmdArgs.push_back("-Bstatic");
  } else {
  ...
  */
  ...
  //We pass file- and library-related args to rigelld in the following order:
  //1) rigel-crt0-mp.ro -- Startup code, this always comes first.
  //                       some architectures also have a crtn.o that comes last.
  //                       The path to this is resolved using LLVM's knowledge of
  //                      ${RIGEL_INSTALL}/target/lib on the file search path.
  //2) System-wide link search path args (e.g., -L${RIGEL_INSTALL}/target/lib)
  //3) User passed object files (.ro), static library filenames (.a) and implicit
  //   libraries (e.g., -lm).  These need to come after the system-wide link search
  //   path if you don't want to have to add -L${RIGEL_INSTALL}/target/lib to everything yourself.
  //4) libc.a -- libc.a depends on libcompiler_rt for double-precision and 64-bit versions of things.
  //   You generally have to pass things in reverse topological order.
  //   That is, if a depends on symbols in b, a must be passed to rigelld *before* b.
  //5) libsim.a -- Provides shims between the target and simulator for syscall implementations on OSless Rigel.
  //6) compiler_rt.a and other sub-libc libraries -- libc may depend on libcompiler_rt for some
  //                                                 things, like 64-bit datatype handling.

  if (!Args.hasArg(options::OPT_nostdlib) &&
      !Args.hasArg(options::OPT_nostartfiles)) {
      CmdArgs.push_back(
            Args.MakeArgString(getToolChain().GetFilePath("rigel-crt0-mp.ro")));
  }

  getRigelToolChain().AddLinkSearchPathArgs(Args, CmdArgs);
  ...
  if (!Args.hasArg(options::OPT_nostdlib) &&
	  ...
	  CmdArgs.push_back("-lpthread"); //libcxxrt and libunwind (a bit) depend on libpthread
		CmdArgs.push_back("-lm"); //I think you're always supposed to link libm for C++ programs.
	}

	//If -pthread was passed and we didn't already pass it as part of C++ support, add
	//-lpthread
	if (Args.hasArg(options::OPT_pthread && !getToolChain().getDriver().CCCIsCXX))
	  CmdArgs.push_back("-lpthread");
	//libc
  CmdArgs.push_back(
          Args.MakeArgString(getToolChain().GetFilePath("libc.a")));
  //libsim is the output of libgloss, which is a shim layer between libc and the bare metal
	//(it mostly provides syscall stubs).
	//We link this unconditionally now, but it would be good to plug different libgloss' in if
	//we ever get an FPGA, substantially different functional simulator, or ASIC target.
  CmdArgs.push_back(
          Args.MakeArgString(getToolChain().GetFilePath("libsim.a")));

	//This should add -lcompiler_rt.
  getRigelToolChain().AddLinkRuntimeLibArgs(Args, CmdArgs);
  ...
  */
  //TODO I don't *think* we need any of this, but not sure.
  /*
  if (!Args.hasArg(options::OPT_nostdlib) &&
      !Args.hasArg(options::OPT_nostartfiles)) {
    if (!Args.hasArg(options::OPT_shared))
  ...
  */
  const char *Exec =
    Args.MakeArgString(getToolChain().GetProgramPath("rigelld"));
  C.addCommand(new Command(JA, *this, Exec, CmdArgs));
}
