...
/// RigelHostInfo
class RigelHostInfo : public HostInfo {

public:
  RigelHostInfo(const Driver &D, const llvm::Triple &Triple);
  ~RigelHostInfo() {}
...
RigelHostInfo::RigelHostInfo(const Driver &D, const llvm::Triple& Triple)
  : HostInfo(D, Triple) {
}

bool RigelHostInfo::useDriverDriver() const { 
  return false;
}

ToolChain *RigelHostInfo::CreateToolChain(const ArgList &Args, 
                                        const char *ArchName) const {
  llvm::Triple TCTriple(getTriple());
//  TCTriple.setArchName(ArchName);
  return new toolchains::RigelToolChain(*this, TCTriple);
}
...
const HostInfo *
clang::driver::createRigelHostInfo(const Driver &D,
                                   const llvm::Triple& Triple) {
  return new RigelHostInfo(D, Triple);
}
