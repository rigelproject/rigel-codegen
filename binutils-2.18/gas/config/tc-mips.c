...
static int file_mips_arch = CPU_RIGEL32; //CPU_UNKNOWN;
...
{
  size_t i;
	//printf("%s [%s]: INFO name: '%s' args: '%s' match: 0x%08lx mask: 0x%08lx\n", 
	//	__FUNCTION__, __FILE__, mo->name, mo->args, mo->match, mo->mask);
...
install_insn (const struct mips_cl_insn *insn)
{
  char *f = insn->frag->fr_literal + insn->where;
	//printf("%s [%s]: Install opcode: 0x%08lx\n", __FUNCTION__, __FILE__, insn->insn_opcode); 
  if (!mips_opts.mips16)
...
//printf("%s [%s]: symbol: '%s'\n", __FUNCTION__, __FILE__, *s);
  ...
  *e = save_c;
  if (regnop) 
    *regnop = reg;
//printf("%s [%s]: reg: 0x%08x\n", __FUNCTION__, __FILE__, reg);
  return reg >= 0;
}
...
//	case 'V':
	...
	case 'D':
	  INSERT_OPERAND (WDimm, insn, va_arg (args, int));
	  continue;

	case 'q':
	  INSERT_OPERAND (RDspr, insn, va_arg (args, int));
	  continue;

	case 'Q':
	  INSERT_OPERAND (RDspr2, insn, va_arg (args, int));
	  continue;

	case 'S':
	  INSERT_OPERAND (RSspr, insn, va_arg (args, int));
	  continue;
...
//	case 'v':
	...
	case 'A':
		INSERT_OPERAND (RA, insn, va_arg (args, int));
		continue;
#if 0
	case 'a':
		INSERT_OPERAND (Ra, insn, va_arg (args, int));
...
#endif
	case 'V':
		INSERT_OPERAND (RV, insn, va_arg (args, int));
		continue;
	...
	//printf("IN %s\n", __FUNCTION__);
	  ...
	  continue;
// BT restored this version for 26bit jump address target
#if 1
	case 'a':
	  assert (ep != NULL);
...
macro (struct mips_cl_insn *ip)
{
  ...
  switch (mask)
    ...
    case M_JAL_A:
		printf("%s (%d): Building M_JAL_A Macro?\n", __FILE__, __LINE__);
      if (mips_pic == NO_PIC)
...
//	  case 'A': USE_BITS (OP_MASK_SHAMT,	OP_SH_SHAMT);	break;
	  ...
	  case 'S': USE_BITS (OP_MASK_RSspr,	OP_SH_RSspr); break;
...
//      case 'A': break;
      ...
      case 'C':	USE_BITS (OP_MASK_COPZ,		OP_SH_COPZ);	break;
		case 'D':	USE_BITS (OP_MASK_WDimm,		OP_SH_WDimm);	break;
 ...
 //     case 'V':	USE_BITS (OP_MASK_FS,		OP_SH_FS);	break;
      ...
      case 'q':	USE_BITS (OP_MASK_RDspr,	OP_SH_RDspr);	break;
      case 'Q':	USE_BITS (OP_MASK_RDspr2,		OP_SH_RDspr2);
      case 'r': USE_BITS (OP_MASK_RS,		OP_SH_RS);	break;
      case 's':	USE_BITS (OP_MASK_RS,		OP_SH_RS);	break;
      case 'A':	USE_BITS (OP_MASK_RA,		OP_SH_RA);	break;
      //case 'a':	USE_BITS (OP_MASK_Ra,		OP_SH_Ra);	break;
      case 'V':	USE_BITS (OP_MASK_RV,		OP_SH_RV);	break;
      case 't':	USE_BITS (OP_MASK_RT,		OP_SH_RT);	break;
      ...
      case '5': {
				USE_BITS (OP_MASK_IMM5, 	OP_SH_IMM5);	break;
			}
...
mips_ip (char *str, struct mips_cl_insn *ip)
{
  ...
  for (;;)
      ...
      if (! ok)
	{
      ...
      insn_error = NULL;
      argnum = 1;
//printf("%s [%s]: args: '%s'\n", __FUNCTION__, __FILE__, insn->args);
      for (args = insn->args;; ++args)
	{
	  int is_mdmx __attribute__ ((unused));
...
do_msbd:
		...
		//  if (regno == AT && ! mips_opts.noat)
		  ...
		  switch (c)
		    ...
		    case 'd':
		      ...
		      break;
		    case 'D':
		      INSERT_OPERAND (WDimm, *ip, regno);
		      break;
		    case 't':
		      INSERT_OPERAND (RT, *ip, regno);
		      break;
			case 'S':
			  INSERT_OPERAND(RSspr, *ip, regno);
			  break;
			case 'q':
			  INSERT_OPERAND(RDspr, *ip, regno);
			  break;
			case 'Q':
			  INSERT_OPERAND(RDspr2, *ip, regno);
			  break;
		    case 'A':
		...
		case '5':
			//printf("%s [%s]: Inserting immediate.\n", __FUNCTION__, __FILE__);
		...
		// HACK: Since the imm is not at offset zero, we have to add it and then
		// make the actual value zero since binutils inserts it elsewhere
		if (*args == '5') {
...
#define OPTION_RIGEL32 (OPTION_ARCH_BASE + 11)
  {"mipsrigel32", no_argument, NULL, OPTION_RIGEL32},
  /* Options which specify Application Specific Extensions (ASEs).  */
#define OPTION_ASE_BASE (OPTION_ARCH_BASE + 12)
...
s_change_section (int ignore ATTRIBUTE_UNUSED)
{
  ...
  int section_alignment __attribute__ ((unused));
  ...
  if (IS_ELF && !ECOFF_DEBUGGING && mips_flag_pdr)
    {
      ...
      valueT dot __attribute__ ((unused));
      expressionS exp;
      char *fragp;
...
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  { "mipsrigel32",       MIPS_CPU_IS_ISA,		ISA_RIGEL32,   CPU_RIGEL32},
...
-mipsrigel32					general RIGEL code???\n\
