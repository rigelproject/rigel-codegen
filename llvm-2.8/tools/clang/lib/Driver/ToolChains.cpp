...
//RigelToolChain -- We use clang for preprocessing, compiling, and optimizing,
//but binutils 2.18 rigelas and rigelld.
RigelToolChain::RigelToolChain(const HostInfo &Host, const llvm::Triple& Triple)
  : ToolChain(Host, Triple) {

  // Get RIGEL_INSTALL value from environment variable
	// If env var is not set, set to empty string (probably resulting in an error)

  const char *RigelInstallC = ::getenv("RIGEL_INSTALL");
  if (!RigelInstallC || RigelInstallC[0] == '\0') {
    RigelInstall = "";
    getDriver().Diag(clang::diag::err_drv_invalid_rigelinstall) << RigelInstall;
  }
  else {
    RigelInstall = RigelInstallC;
  }

  std::string Path;

  // Host binary directory
  Path = RigelInstall + "/host/bin";
  if (llvm::sys::Path(Path).exists()) {
    getProgramPaths().push_back(Path);
  }

  // Target library directory
  Path = RigelInstall + "/target/lib";
  if (llvm::sys::Path(Path).exists()) {
    getFilePaths().push_back(Path);
  }
...
RigelToolChain::~RigelToolChain() {
  for (llvm::DenseMap<unsigned, Tool*>::iterator
           it = Tools.begin(), ie = Tools.end(); it != ie; ++it)
      delete it->second;
}

bool RigelToolChain::IsMathErrnoDefault() const { 
  return true; 
}

bool RigelToolChain::IsUnwindTablesDefault() const {
  return false;
}

const char *RigelToolChain::GetDefaultRelocationModel() const {
  return "static";
}

const char *RigelToolChain::GetForcedPicModel() const {
  return 0;
}

Tool &RigelToolChain::SelectTool(const Compilation &C, 
                            const JobAction &JA) const {
  Action::ActionClass Key;
  if (getDriver().ShouldUseClangCompiler(C, JA, getTriple()))
    Key = Action::AnalyzeJobClass;
  ...
  if (!T) {
    ...
    case Action::CompileJobClass:
      T = new tools::Clang(*this); break;
    case Action::AssembleJobClass:
      T = new tools::rigel::Assemble(*this); break;
    case Action::LinkJobClass:
      T = new tools::rigel::Link(*this); break;
    default:
      assert(false && "Unsupported action for Rigel target.");
    }
  }
...
void RigelToolChain::AddLinkSearchPathArgs(const ArgList &Args,
                                      ArgStringList &CmdArgs) const {
  CmdArgs.push_back(Args.MakeArgString("-L" + RigelInstall + "/target/lib"));
}

void RigelToolChain::AddLinkRuntimeLibArgs(const ArgList &Args,
                                      ArgStringList &CmdArgs) const {
