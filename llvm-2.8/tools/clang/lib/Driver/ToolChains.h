...
class LLVM_LIBRARY_VISIBILITY RigelToolChain : public ToolChain {
public:
  RigelToolChain(const HostInfo &Host, const llvm::Triple& Triple);
  ~RigelToolChain();
  ...
  /// \param CmdArgs [out] - The command argument list to append the paths
  /// (prefixed by -L) to.
  void AddLinkSearchPathArgs(const ArgList &Args,
                                     ArgStringList &CmdArgs) const;

  /// AddLinkRuntimeLibArgs - Add the linker arguments to link the compiler
  /// runtime library.
  void AddLinkRuntimeLibArgs(const ArgList &Args,
                                     ArgStringList &CmdArgs) const;

private:
  mutable llvm::DenseMap<unsigned, Tool*> Tools;
  std::string RigelInstall;
  std::string Rigel;
